...
public class RescheduleReqService {
    ...
    public RescheduleRequest createRequest(Long bookingId){
        ...
        booking.setStatus(Status.RESHEDULE_REQUESTED);
        request.setBooking(booking);
        return rescheduleReqRepo.save(request);
    }
    public RescheduleRequest proposeSlots(Long requestId,LocalDateTime slot1,LocalDateTime slot2,LocalDateTime slot3){
        RescheduleRequest request=rescheduleReqRepo.findById(requestId).orElseThrow(()->new RuntimeException("Request not found"));
    ...
    public Booking acceptSlot(Long requestId,LocalDateTime chosenSlot){
        RescheduleRequest req=rescheduleReqRepo.findById(requestId).orElseThrow(()->new RuntimeException("Request not found"));
        ...
        booking.setStatus(Status.RESHEDULED);
